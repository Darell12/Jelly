...
public class VideosController : BaseJellyfinApiController
{
    ...
    public Task<ActionResult> GetVideoStreamByContainer(
        ...
        [FromQuery] Dictionary<string, string> streamOptions)
    {
    // Guid? elementoId = HttpContext.Items["IdElemento"] as Guid?;
    JObject? info = (JObject?)HttpContext.Items["info"];

    #pragma warning disable CS8602 // Dereference of a possibly null reference.
    if (info.HasValues)
    {
    #pragma warning disable CS8600 // Converting null literal or possible null value to non-nullable type.
    JToken itemIdToken = info["itemId"];
    #pragma warning restore CS8600 // Converting null literal or possible null value to non-nullable type.
    Guid itemId = Guid.Empty;

    if (itemIdToken != null)
    {
        if (Guid.TryParse(itemIdToken.ToString(), out Guid parsedItemId))
        {
            itemId = parsedItemId;
        }
        else
        {
            // Manejar el caso en el que el valor no puede convertirse a Guid
            Console.WriteLine("No se pudo convertir 'itemId' a Guid.");
            return Task.FromResult<ActionResult>(NoContent());
        }
    }

    Console.WriteLine($"Valor de elementoId: {itemId}");
    // Obtén los valores de audioBitrate y videoBitrate como enteros
    int audioBitrate = info["audioBitrate"]?.Value<int>() ?? 0;
    int videoBitrate = info["videoBitrate"]?.Value<int>() ?? 0;
    ...
    return GetVideoStream(
        ...
        tag,
        deviceProfileId,
        info["playSessionId"].ToString(),
        segmentContainer,
        segmentLength,
        minSegments,
        info["mediaSourceId"].ToString(),
        deviceId,
        audioCodec,
        enableAutoStreamCopy,
        allowVideoStreamCopy,
    ...
    else
    {
        ...
        Console.WriteLine("HttpContext.Items:");
        foreach (var key in HttpContext.Items.Keys)
        {
            Console.WriteLine($"{key}: {HttpContext.Items[key]}");
        }
